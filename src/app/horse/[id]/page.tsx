...
import { getDb, getHorseResults } from "@/lib/db";
...
function buildChartData(results: SearchResultRow[]) {
  return results
    ...
    .map((r) => ({
      ...
      score: r.score!,
      label: `${formatDate(r.first_day)} — ${r.show_name} — ${r.competition_name} (${r.person_name})`,
    }))
    .reverse();
}
...
export default async function HorsePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const db = getDb();
  const results = getHorseResults(db, id);

  if (results.length === 0) notFound();

  const horse = results[0];
  ...
  return (
    <main className="min-h-screen bg-zinc-50 px-4 py-10">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-zinc-900">{horse.horse_name}</h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-100 text-left text-xs text-zinc-400 uppercase tracking-wide">
                ...
                <th className="px-4 py-3 hidden sm:table-cell">Reiter</th>
            ...
            <tbody className="divide-y divide-zinc-50">
              {results.map((r) => (
                <tr key={`${r.competition_id}-${r.person_api_id}`} className="hover:bg-zinc-50">
                  ...
                  <td className="px-4 py-2.5 hidden sm:table-cell">
                    <Link
                      href={`/rider/${encodeURIComponent(r.person_api_id)}`}
